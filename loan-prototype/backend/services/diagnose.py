"""
credit_service.py - DIAGNOSTIC VERSION
This version will identify the exact field causing encoding issues
"""
...
def sanitize_for_db(s):
    """Aggressively sanitize string to pure ASCII"""
    if s is None:
        return None
...
def generate_contract(cin, loan_amount, duration, monthly_payment, annual_interest_rate=5.0):
    """Generate loan contract document - PURE ASCII VERSION"""
...
def process_loan_decision(cin, commercial_score, risk_score, loan_amount, duration,
    ...
    """Make final loan decision and save to DB with DIAGNOSTIC INFO"""
    print("\n" + "="*60)
    ...
    # --------------------------
    # DIAGNOSTIC: Print each field before sanitizing
    # --------------------------
    print("\n[DIAGNOSTIC] Fields BEFORE sanitization:")
    print(f"  cin: {repr(cin)} (type: {type(cin).__name__})")
    print(f"  contract_path: {repr(contract_path)} (type: {type(contract_path).__name__})")
    print(f"  amortization_path: {repr(amortization_path)} (type: {type(amortization_path).__name__})")
    print(f"  reason: {repr(reason)} (type: {type(reason).__name__})")
    
    # Try to encode each field to identify the problematic one
    fields_to_test = {
        'cin': cin,
    ...
    print("\n[DIAGNOSTIC] Testing UTF-8 encoding for each field:")
    for field_name, field_value in fields_to_test.items():
        if field_value is None:
            print(f"  {field_name}: None (OK)")
            continue
        try:
            field_str = str(field_value)
            field_str.encode('utf-8')
            ...
            if isinstance(field_value, str):
                print(f"    Bytes: {field_value.encode('latin-1', errors='replace')}")
    ...
    print("\n[DIAGNOSTIC] Fields AFTER sanitization:")
    print(f"  cin_safe: {repr(cin_safe)}")
    print(f"  contract_path_safe: {repr(contract_path_safe)}")
    print(f"  amortization_path_safe: {repr(amortization_path_safe)}")
    print(f"  reason_safe: {repr(reason_safe)}")
    ...
    try:
        ...
        if saved:
            print("SUCCESS: Loan saved successfully")
        else:
            print("WARNING: Failed to save loan")
    except Exception as e:
